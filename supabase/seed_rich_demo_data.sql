  ...
  DECLARE
    ...
    snab_project_photos text[] := ARRAY[
  ...
  BEGIN
    ...
    VALUES (
      ...
      '+77262630101', 'info@zhambyl-stroy.kz', 'ул. Толе би 78',
      'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=256&h=256&fit=crop',
      true, 'verified', 4.6, 11, '990001100001'
    ...
    UPDATE public.companies SET logo_url = CASE bin
      WHEN '990001100002' THEN 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=256&h=256&fit=crop'
      WHEN '990001100003' THEN 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=256&h=256&fit=crop'
      WHEN '990001100004' THEN 'https://images.unsplash.com/photo-1621905251919-48416bd8575a?w=256&h=256&fit=crop'
      WHEN '990001100005' THEN 'https://images.unsplash.com/photo-1504328345606-7bbc0c9d2f90?w=256&h=256&fit=crop'
      WHEN '990001100006' THEN 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=256&h=256&fit=crop'
    END
    WHERE bin IN ('990001100002', '990001100003', '990001100004', '990001100005', '990001100006');
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'market_products') THEN
      FOR ci IN 1..4 LOOP
        ...
        FOR i IN 1..6 LOOP
          ...
          VALUES (
            c_id,
            'Комплектация объекта, ' || city,
            'Поставка и логистика стройматериалов на площадку заказчика',
            (2023 + (i % 3))::text,
            'completed',
            (DATE '2023-01-01' + ((snab_idx * 17) % 900))::date
          )
          RETURNING id INTO proj_id;
          INSERT INTO public.project_images (project_id, image_url, caption, image_role, sort_order) VALUES
            (proj_id, snab_project_photos[1 + ((i - 1) % 6)], 'Отгрузка со склада', 'gallery', 0);

          IF i <= 3 THEN
            INSERT INTO public.projects (company_id, title, description, completion_date, project_phase, start_date)
            VALUES (
              c_id,
              'Доставка на стройплощадку №' || i,
              'Самосвалы, разгрузка манипулятором',
              (2024 + (i % 2))::text,
              'completed',
              (DATE '2024-03-01' + ((snab_idx * 11) % 400))::date
            )
            RETURNING id INTO proj_id;
            INSERT INTO public.project_images (project_id, image_url, caption, image_role, sort_order) VALUES
              (proj_id, snab_project_photos[1 + (i % 6)], 'Разгрузка на объекте', 'site_end', 0);
          END IF;
    ...
    -- 7. Портфолио
    -- =========================================================================
    INSERT INTO public.projects (company_id, title, description, completion_date, project_phase, start_date) VALUES
      ((SELECT id FROM companies WHERE bin = '990001100001'), 'Коттедж 185 м², мкр. Ақ Orda', 'Коробка + кровля', '2025', 'completed', '2024-04-01'),
      ((SELECT id FROM companies WHERE bin = '990001100001'), 'Реконструкция торгового зала', 'Supermarket 340 м²', '2024', 'completed', '2023-08-01'),
      ((SELECT id FROM companies WHERE bin = '990001100001'), 'Школа №12 — капремонт', 'Фасад и кровля, 1180 м²', '2025', 'completed', '2024-09-01'),
      ((SELECT id FROM companies WHERE bin = '990001100002'), 'ЖК «Орталық»', 'Фасад 4100 м²', '2025', 'completed', '2024-02-01'),
      ((SELECT id FROM companies WHERE bin = '990001100002'), 'Бизнес-центр на Абая', 'Вентилируемый фасад, керамогранит', '2024', 'completed', '2023-05-01'),
      ((SELECT id FROM companies WHERE bin = '990001100002'), 'ТРК «Mega» — 2-я очередь', 'Утепление и облицовка', '2026', 'in_progress', '2025-10-01'),
      ((SELECT id FROM companies WHERE bin = '990001100003'), 'Логистический хаб', 'Кровля 2050 м²', '2024', 'completed', '2023-11-01'),
      ((SELECT id FROM companies WHERE bin = '990001100003'), 'Ангар 1480 м²', 'Металлочерепица, водосток', '2025', 'completed', '2024-06-01'),
      ((SELECT id FROM companies WHERE bin = '990001100004'), 'Офисный центр на Достyk', 'Слаботочные сети', '2023', 'completed', '2022-09-01'),
      ((SELECT id FROM companies WHERE bin = '990001100004'), 'Складской комплекс', 'Освещение и СКУД', '2024', 'completed', '2023-12-01'),
      ((SELECT id FROM companies WHERE bin = '990001100005'), 'ТРЦ — VRV система', 'Холодильные центры, 4200 м²', '2025', 'completed', '2024-01-01'),
      ((SELECT id FROM companies WHERE bin = '990001100005'), 'Бизнес-центр Esil', 'Приточно-вытяжная вентиляция', '2024', 'completed', '2023-07-01'),
      ((SELECT id FROM companies WHERE bin = '990001100006'), 'Комплектация ЖК', 'Сыпучие, металл, кровля', '2025', 'completed', '2024-05-01'),
      ((SELECT id FROM companies WHERE bin = '990001100006'), 'Ночная отгрузка 3200 м²', 'Логистика на 12 объектов', '2024', 'completed', '2023-10-01');

    -- Фото портфолио regional demo (если ещё нет)
    INSERT INTO public.project_images (project_id, image_url, caption, image_role, sort_order)
    SELECT p.id,
      (ARRAY[
        'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=800&h=600&fit=crop',
        'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=800&h=600&fit=crop',
        'https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=800&h=600&fit=crop',
        'https://images.unsplash.com/photo-1590496793907-607806659bee?w=800&h=600&fit=crop',
        'https://images.unsplash.com/photo-1621905251919-48416bd8575a?w=800&h=600&fit=crop',
        'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&h=600&fit=crop'
      ])[1 + (abs(hashtext(p.id::text)) % 6)],
      'Объект',
      'gallery',
      0
    FROM public.projects p
    INNER JOIN public.companies c ON c.id = p.company_id
    WHERE c.bin LIKE '990001100%'
  ...
  -- Логотипы базовых test-компаний (если seed_test запускали раньше без logo_url)
  UPDATE public.companies SET logo_url = CASE bin
    WHEN '101240012345' THEN 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?w=256&h=256&fit=crop'
    WHEN '101240023456' THEN 'https://images.unsplash.com/photo-1581094794329-c8112a89af12?w=256&h=256&fit=crop'
    WHEN '101240034567' THEN 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=256&h=256&fit=crop'
    WHEN '101240045678' THEN 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=256&h=256&fit=crop'
  END,
  updated_at = now()
  WHERE bin IN ('101240012345', '101240023456', '101240034567', '101240045678')
